...
"""
        ...
        if '--decode' in self.args:
            self.mode = 'd'
            self.args.remove('--decode')

        if '-i' in self.args:
            self.ignore = True
            self.args.remove('-i')

        if '--ignore-garbage' in self.args:
            self.ignore = True
            self.args.remove('--ignore-garbage')

        if '-w' in self.args:
            self.args.remove(self.args[self.args.index('-w')+1])
            self.args.remove('-w')
...
""" % self.args[0]
            ...
            pname = self.fs.resolve_path(self.args[0], self.protocol.cwd)
